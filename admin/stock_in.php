...
if(isset($_POST['quentity'])){
  ...
  $data1=$obj->db->query('select max(invoice_no) as d from stock_in');
  $data['invoice_no']=(int)$data1->fetch(PDO::FETCH_ASSOC)['d']+1;
  $obj->insert('stock_in',$data);
  // echo "<pre>";
  // var_dump($data);
  // exit();
...
<div class="wrapper">
  ...
  <!-- Main content -->
  <section class="content" style="min-height: 75vh";>
    <div class="row">
      <div class="box">
        <div class="box-body">
  ...
  <tfoot>
    <tr>
      ...
      <th >
        <input type="text" name="sub_total" readonly="" id="sub">
      </th>
      <th></th>
    </tr>
    <tr>
      ...
      <th id="dis">
        <input type="text" required="" name="discount" onkeyup="dis_calculate()" id="d" value="0" class="form-control">
      </th>
      <th></th>
    </tr>
    <tr>
      <th class="text-right" colspan="4">Total</th>
      <th ><input type="text" id="to" readonly="" name="total" class="form-control"></th>
      <th></th>
    </tr>
    <tr>
    ...
    $('#inc').val(i)
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          var ht='<tr id="r_'+i+'">'
          ht+='<td>'+i+'</td>'
          ht+='<td><input type="text" class="form-control" readonly="" value="'+data.name+'"><input type="hidden" name="productid" class="form-control" value="'+data.id+'"></td>'

          ht+='<td><input onkeyup="calculate('+i+')" id="p_'+i+'"  value="'+data.price+'" readonly="" type="text"  class="form-control" ></td>'

          ht+='<td><input onkeyup="calculate('+i+')" id="q_'+i+'" value="0" type="text" autocomplete="off" name="quentity" class="form-control" tabindex="'+i+'"></td>'

          ht+='<td><input id="t_'+i+'" type="text"  value="0" class="form-control tt"></td>'
